...
在React中，事件代理是通过将事件监听器添加到组件树的最外层元素（通常是根节点）来实现的。这种方式利用了浏览器的事件冒泡机制，使得所有事件最终都会冒泡到最外层元素，由该元素上的事件监听器统一处理。以下是其原理和具体实现方式：

#### 原理
1. **事件委托**：React将所有事件监听器绑定到组件树的最外层元素，而不是每个具体的DOM节点上。这样做减少了内存消耗，并简化了事件管理。
2. **合成事件**：React创建了一个合成事件层，它符合W3C标准，与原生的浏览器事件拥有相同的接口。合成事件在事件处理函数执行前后会进行一些额外的处理，如自动绑定`this`到当前组件实例。
3. **事件池**：为了提高性能，React使用了事件池来管理合成事件对象的创建和销毁。事件触发时，React从事件池中获取一个事件对象，处理完成后将其回收，而不是每次都创建新的事件对象。
4. **事件映射**：React维护了一个事件处理函数映射表，记录了每个组件实例及其对应的事件处理函数。当事件发生时，React根据事件类型和目标元素从映射表中查找并执行相应的处理函数。

#### 具体实现方式
1. **事件注册**：在React组件挂载时，React会将所有支持的事件注册到根节点上。这通过`listenToAllSupportedEvents`函数实现，该函数遍历所有原生事件，并在根节点上添加相应的事件监听器。
2. **事件捕获和处理**：当事件发生时，首先被根节点上的事件监听器捕获。然后，React根据事件类型和目标元素，从事件处理函数映射表中查找对应的处理函数，并将合成事件对象作为参数传递执行。
3. **事件冒泡和执行**：React自身实现了一套事件冒泡捕获机制。事件会从触发事件的DOM节点开始，逐层向上冒泡，依次执行各层节点上对应的事件处理函数。在React中，这个过程是通过遍历组件树来实现的。

通过事件代理，React简化了事件处理机制，提高了性能，并解决了跨浏览器兼容性问题。同时，合成事件和事件池的使用进一步优化了内存管理和事件处理的效率。
...
- 使用 React.PureComponent
   React 提供了一个辅助对象来实现浅比较(shallowCompare)这种模式 - 继承自 React.PureComponent。当组件更新时，如果组件的 props 和 state 都没发生改变，render 方法就不会触发，省去 Virtual DOM 的生成和比对过程，达到提升性能的目的。
