...
* **组件内交互导航**：在组件内部根据用户的操作（如按钮点击、表单提交等）来触发导航。例如，在一个登录组件中，登录成功后跳转到主页。
* **动态导航逻辑**：根据组件的状态或其他条件来决定导航的路径。比如，根据用户的角色不同，跳转到不同的页面。

**`redirect`**

* **数据加载和表单处理中的重定向**：在 `loader` 函数中，根据数据加载的结果进行重定向，如未登录用户访问需要权限的页面时重定向到登录页；在 `action` 函数中，根据表单提交的结果进行重定向，如表单提交成功后重定向到成功页面。
* **服务端渲染（SSR）中的重定向**：在 SSR 场景下，`redirect` 可以在服务端直接处理重定向逻辑，避免不必要的客户端渲染和跳转。
