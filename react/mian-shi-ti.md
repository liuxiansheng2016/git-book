...
* **任务拆分与时间切片**\
  ...
  * **任务拆分**：React Fiber把渲染和更新的大任务拆分成一个个较小的任务单元，这些单元被称为Fiber。
  * **优先级调度**：并非所有任务都是同等重要的，React会根据任务的紧急程度来决定哪些任务应该先执行。例如，用户输入、动画等高优先级任务会被优先处理。
  * **可中断与恢复**：由于每个任务单元足够小，React可以在任何时刻暂停当前正在处理的任务，并在稍后继续处理。这种能力允许React在执行过程中检测是否有更高优先级的任务需要处理，并作出相应的调整。
  * **避免阻塞主线程**：传统上，如果React需要进行大量的DOM更新操作，这可能会阻塞浏览器的主线程，造成页面冻结或反应迟钝。使用时间切片后，React可以确保即使在处理大型更新时也不会长时间占用主线程，从而保持页面的流畅性。
* **优先级调度**\
